...
// Returns the alerts created during this scan (also used by POST /api/alerts/generate)
async function generateAlerts() {
  ...
  try {
    ...
    console.log(`[Alert Generator] Scan complete at ${new Date().toISOString()} (${created.length} new alerts)`);
  } catch (err) {
...
const guardedGenerate = nonOverlapping('Alert Generator', generateAlerts);

function startAlertScheduler() {
  // Run on startup after 30 seconds
  setTimeout(() => guardedGenerate(), 30000);
  // Then every 1 hour
  setInterval(() => guardedGenerate(), 60 * 60 * 1000);
